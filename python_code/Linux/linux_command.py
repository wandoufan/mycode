...
# 'cp -r 源文件/文件夹 目标文件/文件夹'复制文件/文件夹
...
# 安装完成后正常运行时是否还需要这个目录，这个目录是否可以像安装包一样删除掉，还是需要移动到/usr下的某个目录，具体应该放在哪个目录下
# 完成安装后会在安装目录下编译产生一个src目录，里面有很多命令，怎么添加到系统命令里，而不是每次都进入该目录下执行命令
