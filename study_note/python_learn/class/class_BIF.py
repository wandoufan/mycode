...
#issubclass(class,classinfo),如果第一个参数是第二个参数的子类，则返回Ture,否则False
class A:
    pass
...
#isinstance(object,classinfo),如果第一个参数是第二个参数或参数子类的实例对象，则返回Ture,否则False
...
#以下函数都是访问对象属性的BIF
#hasattr(object,name),attr是attribute的缩写，测试一个对象里是否有指定的属性，返回True或False
c1 = C()
print(hasattr(c1,'color'))#注意：属性名要用括号括起来！
print(hasattr(c1,'weight'))

#getattr(object,name,default)返回指定对象的属性值，如果属性值不存在则返回自定义的default
#如果没有设置default参数，则返回AttributeError异常
print(getattr(c1,'color'))
print(getattr(c1,'weight','attribute not exist'))

#setattr(object,name,value)设置对象中指定属性的值，如果属性不存在就新建属性并赋值
setattr(c1,'age',5)
print(getattr(c1,'age'))
setattr(c1,'sex','boy')
print(getattr(c1,'sex'))

#delattr(object,name)删除对象中指定的属性，如果属性不存在返回AttributeError异常
delattr(c1,'sex')
